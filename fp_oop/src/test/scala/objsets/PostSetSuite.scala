...
class PostSetSuite extends munit.FunSuite {
  ...
  test("contains: on nonempty set - true") {
    assert(set4c.contains(c))
    assert(set4d.contains(d))
  }

  test("contains: on nonempty set - false") {
    assert(!set4c.contains(d))
    assert(!set4d.contains(new Post("", "", 0)))
  }

  test("contains: on empty set") {
    assert(!set1.contains(new Post("", "", 0)))
  }

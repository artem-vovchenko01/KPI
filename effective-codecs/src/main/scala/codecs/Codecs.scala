  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def processItem(item: Json, maybePreviouslyDecodedItems: Option[List[A]]): Option[List[A]] =
      maybePreviouslyDecodedItems match {
        case None => None
        case Some(list) => decodeAndPrepend(item, list)
      }
...
@main def run(): Unit =
  ...
  val people = parseJson(""" [ { "name":"Artem","age":20 }, { "name":"Josh","age":25 } ] """)

  val authorObj = parseJson(""" { "name" : "William", "surname" : "Shakespeare" } """)
  // Uncomment the following lines as you progress in the assignment
   ...
   println(authorObj.flatMap(_.decodeAs[Author]))
   println(renderJson(Person("Bob", 66)))
   println(people.flatMap(_.decodeAs[List[Person]]))
